...
function! s:update_finish()
  ...
  if s:switch_in()
    ...
    call setline(1, 'Arkaned. Elapsed time: ' . split(reltimestr(reltime(s:update.start)))[0] . ' sec.')
...
function! s:diff()
  ...
  call setline(8, printf('HELLO ARKAN'))
...
command! CPlugInstall  call s:install(<bang>0, [<f-args>])
command! CPlugUpdate   call s:update(<bang>0, [<f-args>])
command! CPlugClean    call s:clean(<bang>0)
command! CPlugStatus   call s:status()
command! CPlugDiff     call s:diff()
command! CPlugSnapshot call s:snapshot(<bang>0, <f-args>)
